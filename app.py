...
def render_sidebar_after_login(user_id: int):
    ...
    # AIキー（ユーザーが自分のキーを入れる場合）
    st.sidebar.markdown("### 🤖 AI設定")
    st.sidebar.caption("ご自身のOpenAIキーを入れると回数制限なしで使えます（任意）。")
    ...
    if user_key.strip():
        st.sidebar.success("ユーザーキー：有効（回数制限なし）")
    else:
        if service_key_ok and not free_used:
            st.sidebar.success("無料：このユーザーは1回だけAIを使えます（キー入力なしでOK）")
        elif service_key_ok and free_used:
            st.sidebar.warning("無料1回は使用済みです（続けるならユーザーキーを入力）")
        else:
            st.sidebar.error("運営側のOpenAIキーが未設定のため、無料実行ができません。")
...
def render_assets_section(user_id: int):
    ...
    with st.container(border=True):
        ...
        with a5:
            other_name = st.text_input("その他名（任意）", value=str(latest["other_name"]) if latest else "不動産", key="a_other_name")
        with a6:
...
/* expander */
details summary, details p, details span, details div {
  color: rgba(0,0,0,0.82) !important;
  opacity: 1 !important;
}
